...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        # 28= (28+2*padding - (3-1)-1) /1 +1 <=> padding=2
        self.convlayer1=nn.Conv2d(1,16,kernel_size=3,stride=1,padding=1)
        self.convlayer2=nn.Conv2d(16,32,kernel_size=3,stride=1,padding=0)
    ...
    def forward(self, x):
        ...
        #block1
        x=F.max_pool2d(F.relu(self.convlayer1(x)),kernel_size=2, stride=2)
        #using formula in https://pytorch.org/docs/1.9.1/generated/torch.nn.Conv2d.html

        # cc1 -> (28 +2*1 -(3-1) - 1)/1 + 1 -> out:16*28*28

        #using formula in https://pytorch.org/docs/stable/generated/torch.nn.MaxPool2d.html

        #maxpool1 -> (28-(2-1)-1)/2+1   -> out: 16*14*14

        #block2
        x=F.max_pool2d(F.relu(self.convlayer2(x)),kernel_size=2, stride=2)
        #cc2-> 14-3+1=12 -> out: 32*12*12
        #maxpool2 -> (12-(2-1)-1)/2 +1 = 6 -> out: 32*6*6
        x = x.view(-1, 32*6*6)
        #1st affine
        x=F.relu(self.fc1(x))

        #p= default deffined in main (the skelaton class doesnt has arguemnts)
        x=F.dropout(x,p=0.3)
        #2nd affine
        x=F.relu(self.fc2(x))
        # 3rd affine + log_softmax transform
        x=self.fc3(x)
