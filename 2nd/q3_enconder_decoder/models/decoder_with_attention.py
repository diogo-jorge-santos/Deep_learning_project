...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        attn_hidden = self.decoder_att(decoder_hidden.unsqueeze(1).repeat(1, encoder_out.size(1), 1))+self.encoder_att(encoder_out)
        attn_hidden=self.relu(attn_hidden)
        
        attn_scoring_vector=self.full_att(attn_hidden)

        probabilities=self.softmax(attn_scoring_vector)
        
        return torch.bmm(encoder_out.transpose(2,1),probabilities).squeeze(2)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out):
        #TODO: your code here
        embeded=self.embedding(word)

        attention=self.attention(encoder_out,decoder_hidden_state)

        embeded=torch.cat((embeded,attention),dim=1)
        decoder_hidden_state , decoder_cell_state =self.decode_step(embeded,(decoder_hidden_state,decoder_cell_state))
        
        scores=self.dropout(decoder_hidden_state)
