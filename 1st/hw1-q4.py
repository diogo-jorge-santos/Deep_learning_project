...
def main():
    ...
    '''
    code used for logging results
    parser.add_argument('-aux_string', default='',
                        help="Name for plots.")
    '''
    ...
    '''
    code used for logging the results

    args=parser.parse_args()
    aux_string=args.aux_string

    
    file=open("log.txt",mode='a')
    print(aux_string +' Valid acc : ' + str(valid_accs[-1]) + aux_string ,file=file)

    plot(epochs, train_mean_losses, ylabel='Loss', name='training-loss'+ aux_string)
    plot(epochs, valid_accs, ylabel='Accuracy', name='validation-accuracy'+ aux_string)
